...
if(isset($_POST['atenant'])){
	$Tname=$_POST['Tname'];
  $NationalID=$_POST['nationalID'];
  $temail=$_POST['temail'];
  $PhoneNo=$_POST['PhoneNo'];
  $apartment=$_POST['apartment'];
	$houseno=$_POST['houseno']; //ID
  $houseocc=$_POST['houseocc'];
  $Rent=$_POST['Rent'];
  $Deposit=$_POST['Deposit'];
  $cemail=$_POST['cemail'];


  $query = "SELECT * FROM doctb WHERE email='$cemail'";
  $result = mysqli_query($con,$query);
  $row = mysqli_fetch_assoc($result);
  $apartID = $row['spec'];


  $sql = "SELECT * FROM housetb WHERE apartmentID='$apartID' and id='$houseno'";
  ...
  if($password==$cpassword){
  	$query="insert into tenanttb(Tname,NationalID,temail,password,PhoneNo,apartment,houseno,Rent,Deposit,cemail,houseocc,date) values ('$Tname','$NationalID','$temail','$password','$PhoneNo','$apartID','$housenoID','$Rent','$Deposit','$cemail','$houseocc','$date');";
    $result=mysqli_query($con,$query);
    if($result){
      ...
      $tid=$row['Tid'];
      $date = $row['date'];
      $query = "UPDATE housetb
                SET status='Occupied'
                WHERE id='$housenoID' and apartmentID='$apartID'";
      $result= mysqli_query($con,$query);
      $query1 = "INSERT INTO paymentstatustb(tid,currentdate,status) VALUES ('$tid','$date','Not Paid')";
      $result1 = mysqli_query($con,$query1);
      echo "<script>alert('Tenant added successfully!');</script>";
      header("Location:caretaker-panel.php#list-doc");
    }else{
      echo "<script>alert('Could not add tenant successfully!');</script>";
      header("Location:caretaker-panel.php#list-doc");
    }
